...
// Global Valiables
static uint8_t adc_data[ADC_CHANNELS] = {0x20,0x21,0x22,0x23};
static uint16_t tx_data[2] = {0,0};
...
static void onI2CWrite(uint8_t *pbuf, uint8_t len){
  ...
  if((data >= REGISTER_MIN)&&(data <= REGISTER_MAX)){
    currentCh = data- 0x10;
    i2cs.setTxBuffer((uint8_t *)(&tx_data[currentCh]),TX_BUFFER_SIZE);
  }  
}
...
void loop() {
  ...
  for(cnt = 0;cnt < ADC_CHANNELS;cnt ++){
    ...
    data = (uint8_t)(adc >> 2);
    adc_data[cnt] += (data - adc_data[cnt]) / 2;
  }
  tx_data[0] = (adc_data[1] << 8) | adc_data[0];
  tx_data[1] = (adc_data[3] << 8) | adc_data[2];
